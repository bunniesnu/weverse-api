...
func (w *Weverse) GetCommunityArtists(communityId int) ([]CommunityArtistInfo, error) {
	target_path := fmt.Sprintf("/member/v1.1/community-%d/artistMembers", communityId)
	queryParams := map[string]string{
		"fields": "artistCode,artistName,artistImageUrl,artistOfficialProfile",
		"appId":  WeverseWebAppId,
		"wpf":    "pc",
	}
	resp, err := w.weverseAPICall(http.MethodGet, target_path, queryParams, nil)
	...
	data := make([]CommunityArtistInfo, 0)
	if err := json.NewDecoder(reader).Decode(&data); err != nil {
		return nil, fmt.Errorf("error decoding response: %v", err)
	}
	return data, nil
